                            ...
                            <div x-data="{ open: false }" class="relative w-full">
                                <div class="relative">
                                    ...
                                    <div wire:loading wire:target="region_selector.keyword"
                                        class="absolute top-3 right-3 animate-spin inline-block size-4 border-3 border-current border-t-transparent text-blue-500 rounded-full dark:text-blue-500"
                                        ...
                                        @foreach ($this->regions as $region)
                                            <li class="p-2 cursor-pointer hover:bg-gray-100">
                                                <label for="region-{{ $region->code }}"
                                                    ...
                                                    <input type="radio" id="region-{{ $region->code }}"
                                                        value="{{ $region->code }}"
                                ...
                                <label for="shipping_method_{{ $shipping_method->hash }}"
                                    ...
                                    <div class="flex items-center justify-start gap-2">
                                        <input wire:key="{{ $shipping_method->hash }}"
                                            wire:model.live="shipping_selector.shipping_selected" type="radio"
                                            name="shipping_method" value="{{ $shipping_method->hash }}"
                                        ...
                                        <span
                                            class="text-sm text-gray-500 ms-3 dark:text-neutral-400">{{ $shipping_method->label }}
                                        </span>
                                    </div>
                        ...
                        @foreach ($this->payment_methods->toCollection() as $key => $payment_method)
                            <label for="payment_method_{{ $payment_method->hash }}"
                                ...
                                <input type="radio" name="hs-vertical-radio-in-form"
                                    wire:key="payment_method_{{ $payment_method->hash }}"
                        ...
                        <li
                            ...
                            <div class="flex items-center justify-between w-full">
                                <span class="flex flex-col">
                                    <span>{{ $this->shipping_method?->service ?? '-' }}</span>
                                    <span class="text-sm text-gray-600">{{ $this->shipping_method?->label }}</span>
                            ...
                            <div wire:loading wire:target="shipping_selector.shipping_selected"
                                class="animate-spin inline-block size-4 border-3 border-current border-t-transparent text-blue-500 rounded-full dark:text-blue-500"
                                role="status" aria-label="loading">
                                <span class="sr-only">Loading...</span>
                            </div>
