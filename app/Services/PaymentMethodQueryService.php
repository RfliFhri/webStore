...
class PaymentMethodQueryService {
    ...
    {
        return collect($this->drivers)->first(fn(PaymentDriverInterface $driver_interface) => $driver_interface->driver === $payment_data->driver);
    }

    public function getPaymentMethods() : DataCollection
    {
        return collect($this->drivers)
            ->flatMap(fn(PaymentDriverInterface $driver_interface) => $driver_interface->getMethods()->toCollection())
